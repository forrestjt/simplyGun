...
function _sjs(){
  ...
  this.open = function(target, w, h){
    if(w == undefined && h == undefined){v=getViewportSize();w=v.w;h=v.h;}
    ...
    _this.width = t.width = w;
    _this.height = t.height = h;
    t.style.width    = w+"px";
    t.style.height   = h+"px";
  ...
  this.keyUp = function(key, callback){
    window.onkeyup = function(evt){
      if(evt.keyCode == key)callback();
    }
  }
